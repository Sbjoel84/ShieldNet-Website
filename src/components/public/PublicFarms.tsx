...
import { Wheat, TrendingUp, TrendingDown, Minus, ShieldCheck, X } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Badge } from '@/components/ui/badge'
import { Card, CardContent } from '@/components/ui/card'
import { SEED_MARKET_PRICES, SEED_APPLICATIONS } from '@/data/seedData'
...
export function PublicFarms() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', phone: '', crop_type: '', farm_size_ha: '', challenge: '', message: '' })
            ...
            {SEED_MARKET_PRICES.map(mp => (
              <Card key={mp.id} className="hover:border-border/80 transition-colors">
                <CardContent className="p-4">
                  ...
                  <p className="text-xl font-bold text-foreground">₦{mp.price_per_kg.toLocaleString()}<span className="text-xs text-muted-foreground font-normal">/kg</span></p>
                  <p className="text-xs text-muted-foreground mt-1">{mp.market}</p>
                  <div className="flex items-center justify-between mt-2">
                    <Badge variant="blue" className="text-[10px]">{mp.city}</Badge>
                    <span className={`text-[10px] font-medium ${mp.change_percent > 0 ? 'text-green-400' : mp.change_percent < 0 ? 'text-red-400' : 'text-yellow-400'}`}>
      ...
      {formOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl overflow-y-auto max-h-[90vh]">
            <button onClick={() => setFormOpen(false)} className="absolute top-3 right-3 w-7 h-7 rounded-full bg-muted flex items-center justify-center hover:bg-accent transition-colors">
              <X className="w-4 h-4" />
            </button>
            {done ? (
              <div className="flex flex-col items-center gap-3 py-8 text-center">
                ...
                <Button variant="shield" onClick={() => { setFormOpen(false); setDone(false) }}>Done</Button>
              </div>
            ) : (
              <>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.crop_type} onChange={f('crop_type')}>
                      <option value="">Crop Type</option>
                      {CROPS.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <Input placeholder="Farm size (ha)" type="number" value={form.farm_size_ha} onChange={f('farm_size_ha')} />
                  </div>
                  <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.challenge} onChange={f('challenge')}>
                ...
                <div className="flex gap-2 mt-5">
                  ...
                  <Button variant="shield" className="flex-1" onClick={submit} disabled={!form.full_name || !form.email || !form.phone}>Submit</Button>
                </div>
              </>
            )}
          </div>
