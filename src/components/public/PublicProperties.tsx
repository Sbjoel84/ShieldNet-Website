...
import { ShieldCheck, AlertTriangle, Bed, Bath, Maximize2, MapPin, Search, SlidersHorizontal, X, Phone, FileText, User, Calendar, Loader2 } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-border rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <button type="button" onClick={onClose} className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-black/50 flex items-center justify-center text-white hover:bg-black/70 transition-colors">
          <X className="w-4 h-4" />
        </button>
...
function InquiryForm({ property, onClose }: { property: Property; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <button type="button" onClick={onClose} className="absolute top-3 right-3 w-7 h-7 rounded-full bg-muted flex items-center justify-center hover:bg-accent transition-colors">
          <X className="w-4 h-4" />
        </button>
        {done ? (
          ...
          <>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <select aria-label="Interest type" className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.interest_type} onChange={f('interest_type')}>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <Button variant="shield" className="flex-1" onClick={submit} disabled={!form.full_name || !form.email || !form.phone || sending}>
                {sending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Submit Inquiry'}
              </Button>
            </div>
          </>
        )}
    ...
    <div>
      ...
      <div className="bg-gradient-to-b from-blue-500/10 to-transparent border-b border-border py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
          <div className="relative w-28 h-28 rounded-2xl overflow-hidden mx-auto mb-6 shadow-xl shadow-blue-500/20 ring-2 ring-blue-500/30 animate-shield-pulse">
            <img src="/ShieldHome.png" alt="ShieldHome" className="w-full h-full object-contain bg-white" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        ...
        <p className="text-sm text-muted-foreground mb-4">{loading ? 'Loading…' : `${filtered.length} listing${filtered.length !== 1 ? 's' : ''} found`}</p>
