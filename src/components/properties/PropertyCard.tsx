import { Bed, Bath, Maximize2, MapPin, ShieldCheck, AlertTriangle, Eye, Pencil, Trash2, BadgeCheck } from 'lucide-react'
...
const STATUS_LABELS: Record<string, string> = {
  pending: 'Pending', approved: 'Approved', rejected: 'Rejected', flagged: 'Flagged', sold: 'Sold',
}

export function PropertyCard({ property: p, onReport, onView, onEdit, onDelete, onMarkSold }: PropertyCardProps) {
  return (
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between pt-1 border-t border-border">
          <div>
            ...
            {p.status === 'sold' && (
              <Badge className="mt-1 text-[10px] bg-purple-600/20 text-purple-300 border-purple-500/30">Sold</Badge>
            )}
          </div>
          <div className="flex gap-1.5">
            {onReport && (
              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-orange-400" title="Report fraud" onClick={() => onReport(p.id)}>
        ...
        {/* Admin actions */}
        {(onEdit || onDelete || onMarkSold) && (
          <div className="flex gap-1.5 pt-2 border-t border-border">
            {onEdit && (
              <Button variant="outline" size="sm" className="h-7 text-xs flex-1" onClick={() => onEdit(p)}>
                <Pencil className="w-3 h-3 mr-1" />Edit
              </Button>
            )}
            {onMarkSold && p.status !== 'sold' && (
              <Button variant="outline" size="sm" className="h-7 text-xs flex-1 text-purple-400 border-purple-500/30 hover:bg-purple-500/10" onClick={() => onMarkSold(p.id)}>
                <BadgeCheck className="w-3 h-3 mr-1" />Sold
              </Button>
            )}
            {onDelete && (
              <Button variant="outline" size="sm" className="h-7 text-xs text-red-400 border-red-500/30 hover:bg-red-500/10" onClick={() => onDelete(p.id)}>
